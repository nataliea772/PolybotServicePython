...
class TestImgConcat(unittest.TestCase):
    ...
    def test_percentage_untouched_pixels(self):
        squared_diff_sum = sum(
            pixel1 == pixel2 for row1, row2 in zip(self.original_img.data, self.img.data) for pixel1, pixel2 in
            zip(row1, row2))
        untouched_pixel_percentage = (squared_diff_sum / (
                    len(self.original_img.data) * len(self.original_img.data[0]))) * 100
